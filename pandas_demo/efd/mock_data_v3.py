...
logging.basicConfig(
    ...
    filename='efd_data_processing_v3.log'
)
...
# 文件路径配置
SQL_FILE_PATH = os.path.join(BASE_DIR, 'efd', 'rptdw', 'rptdw_v3.sql')
...
class DataProcessor:
            ...
            for table_name, metadata in self.table_metadata.items():
                try:
                    ...
                    if len(df) == expected_rows:
                        ...
                        logger.info(f'    列名: {list(df.columns)[:5]}...' if len(
                            df.columns) > 5 else f'    列名: {list(df.columns)}')
    ...
    def write_to_clickhouse(self):
        ...
        try:
            ...
            # 需要转换为 DateTime 的字段列表（用于分区、ReplacingMergeTree 版本列的字段）
            datetime_fields = {
                'ods_mes_wip_lot_hld': ['created'],
                'dwd_panel_inout_his': ['trans_time'],
                'dwd_panel_scrap_his': ['trans_time'],
                'ods_mes_wip_wo': ['update_time'],
                'dwd_panel_a3g70_defect': ['trans_time'],
                'dwd_panel_a4710_defect': ['trans_time'],
                'ods_mes_prd_part_defect_code': ['update_time'],  # ReplacingMergeTree 版本列
                'ods_mes_ad_user': ['update_time'],  # ReplacingMergeTree 版本列
                'ods_mes_wip_lot': ['updated']  # ReplacingMergeTree 版本列
            }

            # 不能为空的字段列表（用于 ORDER BY 的非 Nullable 字段）
            non_nullable_fields = [
                'panel_id',      # dwd_panel_inout_his, dwd_panel_scrap_his, dwd_panel_a3g70_defect, dwd_panel_a4710_defect
                'user_name',     # ods_mes_ad_user
                'lot_id',        # ods_mes_wip_lot
                'time_key',      # dim_calendar
                'time_type',     # dim_calendar
                'date_key',      # dwd_panel_scrap_his, dwd_panel_a3g70_defect, dwd_panel_a4710_defect
                'hour_key',      # dwd_panel_scrap_his
                'doc_id'         # ods_mes_wip_wo
            ]
            ...
            for table_name, df in self.processed_data.items():
                ...
                try:
                    ...
                    # 获取当前表需要转换为 DateTime 的字段
                    table_datetime_fields = datetime_fields.get(table_name, [])
    
                    # 处理每一列
                    for col in df_copy.columns:
                        col_lower = col.lower()
    
                        # object_rrn 是主键，保持 Int64 类型
                        if col_lower == 'object_rrn':
                            df_copy[col] = pd.to_numeric(df_copy[col], errors='coerce').fillna(0).astype('Int64')
                        # 检查是否是 DateTime 字段
                        elif col_lower in table_datetime_fields:
                    ...
                    for _, row in df_copy.iterrows():
                        ...
                        for idx, val in enumerate(row):
                            ...
                            # object_rrn 必须是整数
                            if col_name == 'object_rrn':
                                try:
                                    row_data.append(int(val) if pd.notna(val) else 0)
                                except:
                                    row_data.append(0)
                            # DateTime 字段已经是时间戳
                            elif col_name in table_datetime_fields:
                                try:
                                    # DateTime 字段不能为 None，使用 0 作为默认值（1970-01-01）
                                    if pd.notna(val):
                                ...
                                if val is None or (hasattr(pd, 'isna') and pd.isna(val)):
                                    # 检查是否是非 Nullable 字段
                                    if col_name in non_nullable_fields:
                                        row_data.append('UNKNOWN')  # 非 Nullable 字段使用 'UNKNOWN' 作为默认值
                                    else:
                                        row_data.append('')  # Nullable 字段使用空字符串
                                else:
...
if __name__ == '__main__':
    ...
    if success:
        print('✅ 数据处理成功完成！')
        print('详细信息请查看日志文件: efd_data_processing_v3.log')
    else:
        print('❌ 数据处理失败，请查看日志文件了解详情。')
        print('日志文件位置: efd_data_processing_v3.log')
    print('=' * 80)
